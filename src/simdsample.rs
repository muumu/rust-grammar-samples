...
fn load_store() {
    ...
    unsafe {
        ...
        // コンパイラの最適化によって高速化されるケースも多いため使用価値は十分にあり
        // 上位64bitに3、下位32bitに1をセット
        // epi64の末尾にxをつけるかつけないかはただの別名で、使用しているコンパイラで存在する方を使えばOK
        let a = _mm_set_epi64x(3, 1);
        // 上位64bit、下位64bitそれぞれに3をセット
        let b = _mm_set1_epi64x(3);
        // 他に32bit、16bit、8bit用もあり
